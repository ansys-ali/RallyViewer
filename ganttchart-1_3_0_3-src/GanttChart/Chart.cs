...
namespace Braincase.GanttChart
{
    ...
    public partial class Chart : UserControl
    {
        ...
        protected virtual void OnTaskMouseDoubleClick(TaskMouseEventArgs e)
        {
            ...
            if (e.Button == System.Windows.Forms.MouseButtons.Left)
            {
                ...
                e.Task.Collapse(e.Task.IsCollapsed);
            }
            else if (e.Button == System.Windows.Forms.MouseButtons.Right)
            {
        ...
        private void _GenerateModels()
        {
            ...
            foreach (var task in _mProject.Tasks)
            {
                ...
                if (!_mProject.AncestorsOf(task).Any(x => x.IsCollapsed))
                {
                    ...
                    taskRect = new RectangleF(task.Start * this.BarWidth + this.BarWidth / 2, y_coord, task.Duration * this.BarWidth, this.BarHeight * task.Sizing);
        ...
        private void _DrawHeader(Graphics graphics, Rectangle clipRect)
        {
            ...
            var e = new HeaderPaintEventArgs(graphics, clipRect, this, new Font(this.Font.FontFamily, SystemFonts.DefaultFont.Size), this.HeaderFormat);
            if (PaintHeader != null)
                PaintHeader(this, e);
        ...
        private int _DrawTasks(Graphics graphics, Rectangle clipRect)
        {
            ...
            foreach (Task task in _mChartTaskRects.Keys)
            {
                // Get the taskrect
                ...
                {
                    TaskFormat format = this.TaskFormat;
                    Font font = this.Font;

                    // Crtical Path
                    bool critical = crit_task_set.Contains(task) || task.IsHighlighted;
                    if (critical)
                    {
                        format = this.CriticalTaskFormat;
                        format.Color = Brushes.MediumSlateBlue;
                        font = new Font(this.Font, FontStyle.Bold);
                    }

                    if (task.HeaderSize == HeaderSize.Big)
                    {
                        format.BackFill = Brushes.Black;
                        format.Color = Brushes.White;
                    }
                    else if (task.HeaderSize == HeaderSize.Medium)
                    {
                        font = new Font(this.Font, FontStyle.Bold);
                    }
                    ...
                    if (task.IsAccepted)
                    {
                        format.ForeFill = Brushes.DarkGreen;
                    }
                    if (task.IsSelected)
                    {
                        format.Border = new Pen(Color.Red, format.Border.Width * 3);
                    }

                    e = new TaskPaintEventArgs(graphics, clipRect, this, task, row, font, format);
        ...
        private void __DrawRegularTaskAndGroup(Graphics graphics, TaskPaintEventArgs e, Task task, RectangleF taskRect)
        {
            ...
            if (task.Children.Count > 0)
            {
                ...
                if (task.IsCollapsed)
                    graphics.DrawString("▶", font, Brushes.White, taskRect);
                else
                    graphics.DrawString("⍔", font, Brushes.White, taskRect);
            }
        }
    ...
    public class TaskPaintEventArgs : ChartPaintEventArgs
    {
        ...
        /// </summary>
        public TaskPaintEventArgs(Graphics graphics, Rectangle clipRect, Chart chart, Task task, int row, Font font, TaskFormat format) // need to create a paint event for each task for custom painting
            : base(graphics, clipRect, chart)
        {
            this.Task = task;
            this.Row = row;
