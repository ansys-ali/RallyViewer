...
namespace Rally.RestApi.Test
{
	...
	{
		public static RallyRestApi GetRallyRestApi(string userName = "art.li@ansys.com", string password = "Adm.net1",
			string server = "https://rally1.rallydev.com", string wsapiVersion = "")
		{
		...
		public void GetCurrentUser()
		{
            ...
            Assert.AreEqual("art.li@ansys.com", user.UserName, "Name test");
        }
        ...
        public void GetStories()
        {
            ...
            //request.Project = "project/3961858474";
            Query sagaQuery = new Query("FormattedID", Query.Operator.Equals, "SF3055");
            request.Query = sagaQuery;

            QueryResult result = restApi.Query(request);
            Assert.IsTrue(result.Success, "Query Success");

            List<DynamicJsonObject> userStoryCollection = new List<DynamicJsonObject>();
            foreach (var feature in result.Results)
            {
                userStoryCollection.Add(feature.UserStories);
            }

            List<DynamicJsonObject> list = new List<DynamicJsonObject>();
            foreach (var collection in userStoryCollection)
            {
                var storyRequest = new Request(collection);
                var storyResults = restApi.Query(storyRequest);

                foreach (var userStory in storyResults.Results)
                {
                    list.Add(userStory);
                }
            }

            if (result.TotalResultCount > request.Limit)
                Assert.AreEqual(request.Limit, result.Results.Count());
            else
                Assert.AreEqual(result.TotalResultCount, result.Results.Count());
        }
